...
class PredictionService:
    ...
    def predict_tickets(self, start_date: datetime, end_date: datetime) -> List[Dict]:
        return self._predict_helper(lambda: self.predictor.predict_range(start_date, end_date))
    ...
    def get_health_status(self) -> Dict:
        try:
            ...
            model_info = {
                ...
                'data_points': len(self.predictor.daily_df) if self.predictor and hasattr(self.predictor, 'daily_df') else 0
            }
            return {
                'status': status,
