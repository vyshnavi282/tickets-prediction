...
print("📊 Model Evaluation Results (XGBoost train API)")
...
# Function to predict multiple preset durations
def predict_multiple_durations(bst, scaler, daily_df, holiday_flags, major_event_flags):
    last_date = daily_df['start_date'].max()
    durations = [1, 2, 3, 4, 5, 6, 7, 30]
    results = {}

    for days in durations:
        start = last_date + pd.Timedelta(days=1)
        end = last_date + pd.Timedelta(days=days)
        preds = predict_range(bst, scaler, daily_df, start, end, holiday_flags, major_event_flags)
        results[f'next_{days}_days'] = preds

    return results

# Get predictions for all durations
all_predictions = predict_multiple_durations(bst, scaler, daily_df, holiday_flags, major_event_flags)

# Print predictions
for duration, df_pred in all_predictions.items():
    print(f"\nPrediction for {duration.replace('_', ' ')}:")
    print(df_pred)
